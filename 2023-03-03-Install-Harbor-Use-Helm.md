---
title: 通过 Helm chart 安装 Harbor
thumbnailImagePosition: left
thumbnailImage: https://img.freepik.com/premium-photo/multi-exposure-woman-hands-typing-computer-financial-graph-hologram-drawing-stock-market-analysis-concept_700248-45167.jpg
date: 2023-03-03
categories:
- Harbor
- Helm
...
配置证书，该证书使用自签名的证书，通过 OpenSSL 生成
```basH
root@master:~# kubectl create secret generic harbor-tls --from-file=harbor.pem --from-file=harbor-key.pem --from-file=ca.pem -n harbor
```

```bash
root@master:~# helm pull harbor/harbor
root@master:~# tar -zxf harbor-1.13.0.tgz
oot@master:~# ls harbor
ca  Chart.yaml  LICENSE  README.md  templates  values.yaml
```

values.yaml 关键配置选项说明。该示例用于拥有 ingress 的环境，如果你的环境没有 ingress，你可以使用 nodePort 的方式暴露你的服务。该文件也已经配置好的 nodePort 相关的字段，只需要将 expose.type 的值改成 nodePort 即可。
```bash
root@master:~# cat harbor/values.yaml
expose:
  # Set how to expose the service. Set the type as "ingress", "clusterIP", "nodePort" or "loadBalancer"
  # and fill the information in the corresponding section
  type: ingress          # 配置服务暴露的方式，其值可以是 "ingress", "clusterIP", "nodePort" 或 "loadBalancer"
  tls:
    # Enable TLS or not.
    ...
    enabled: true            # 如果值为 true 则表示启用 SSL
    ...
    auto:
      ...
      commonName: "harbor.test.local"           # SSL 证书对应的主机名，要与 externalURL 一致
    secret:
      # The name of secret which contains keys named:
      # "tls.crt" - the certificate
      # "tls.key" - the private key
      secretName: "harbor-tls"                # 证书对应的 secret 名称，前面通过 kubectl create secret tls 命令创建的
  ingress:                 # 该部分选项只有 expose.type 类型为 Ingress 才需要配置
    hosts:
      core: harbor.test.local     # ingress 的主机名，与 externalURL 一致
    ...
    harbor:
      ...
      labels: {}
  clusterIP:          # 该部分选项只有 expose.type 类型为 clusterIP 才需要配置
    ...
    ports:
      ...
      httpsPort: 443
  nodePort:        # 该部分选项只有 expose.type 类型为 nodePort 才需要配置
    ...
    ports:
      http:
        ...
        nodePort: 30008         # http 对应的 node port
      https:
        # The service port Harbor listens on when serving HTTPS
        port: 443
        # The node port Harbor listens on when serving HTTPS
        nodePort: 30009    # https 对应的 node port
  loadBalancer:        # 该部分选项只有 expose.type 类型为 loadBalancer 才需要配置
...
externalURL: https://harbor.test.local .com:443     # 外部访问地址
...
persistence:    # 数据持久化
  ...
  resourcePolicy: "keep"
  persistentVolumeClaim:             # PVC 配置，为每个节点配置 PVC（existingClaim）
    registry:
      # Use the existing PVC which must be created manually before bound,
      # and specify the "subPath" if the PVC is shared with other components
      existingClaim: "harbor-pvc"             # 对应的 PVC 名
      # Specify the "storageClass" used to provision the volume. Or the default
      # StorageClass will be used (the default).
      # Set it to "-" to disable dynamic provisioning
      storageClass: ""
      subPath: "registry"    # PVC 中的子目录，因为我将所有节点的持久化都放到同一个 PVC 了，所以这里需要添加 subPath 区分
...
创建 PV 和 PVC。因为是测试，我将所有的数据持久化都放到了同一个 PVC，通过子目录的方式，存放不同节点的数据持久化。具体可以参考上面 value.yaml 文件中的 persistence 部分
```bash
...
harbor-pv   15Gi       RWX            Retain           Bound    harbor/harbor-pvc                           10d
root@master:~# kubectl get pvc -n harbor
NAME         STATUS   VOLUME      CAPACITY   ACCESS MODES   STORAGECLASS   AGE
harbor-pvc   Bound    harbor-pv   15Gi       RWX                           10d
```

使用配置好的 value.yaml 文件部署 harbor
```bash
root@master:~# helm install harbor harbor/harbor -f values.yaml -n harbor
root@master:~# kubectl get pods -n harbor
NAME                                 READY   STATUS    RESTARTS      AGE
harbor-core-64f6c68ff6-p8rjr         1/1     Running   0             10d
harbor-database-0                    1/1     Running   0             10d
harbor-jobservice-6844d5686b-hhjsb   1/1     Running   0             10d
harbor-portal-f766bccf9-2vj88        1/1     Running   0             10d
harbor-redis-0                       1/1     Running   0             10d
harbor-registry-644445c89f-p998j     2/2     Running   0             10d
harbor-trivy-0                       1/1     Running   0             10d
```

配置好 DNS 解析，现在我们可以通过 externalURL 来访问我们的 harbor 了。
